...
# Professional visualization tools for epidemiological models
...
plot_sir_results <- function(sir_data, title = "SIR Model Simulation", show_peak = TRUE) {
  ...
  # Add peak infection marker if requested
  if (show_peak && max(sir_data$I) > 0) {
    ...
    p <- p + 
      geom_point(aes(x = peak_day, y = peak_value), 
                 color = "#C00000", size = 3, shape = 21, fill = "white") +
      geom_text(aes(x = peak_day, y = peak_value, 
                    label = paste("Peak:", round(peak_value))), 
